...
\begin{equation}
\begin{aligned}
    \using l_X &= \min(4, \instructions_{\imath+1}) \,,\quad&
    \immed_X &\equiv \sext_{l_X}(\de_{l_X}(\instructions_{\imath+2\dots+l_X})) \\
    \using l_Y &= \min(4, (\ell - l_X - 1) \bmod 2^8) \,,\quad&
...
\begin{aligned}
    ...
    \using l_X &= \min((\ell - 1) \bmod 2^8, 4) \,,\quad&
    \immed_X &\equiv \sext_{l_X}(\de_{l_X}(\instructions_{\imath+2\dots+l_X}))
\end{aligned}
\end{equation}
...
\begin{aligned}
    ...
    \reg_A &\equiv \reg_{r_A} \,,\quad
    \reg'_A \equiv \reg'_{r_A} \\
    \using l_X &= \min(4, \ffrac{\instructions_{\imath+1}}{16}) \,,\quad&
...
\begin{equation}
  \begin{aligned}
    ...
    \reg_B &\equiv \reg_{r_B} \,,\quad
    \reg'_B \equiv \reg'_{r_B} \\
    \using l_X &= \min(4, \instructions_{\imath+2}) \,,\quad&
    \immed_X &= \sext_{l_X}(\de_{l_X}(\instructions_{\imath+3\dots+l_X})) \\
    \using l_Y &= \min(4, (\ell - l_X - 2) \bmod 2^8) \,,\quad&
